...
# Compute k(x_*, x_*)
k_star_star = kernel(X_test[0], X_test[0])  # Should be 1
...
# Step 7: Compute the Predictive Variance
v = np.linalg.solve(K, k_star)
sigma_star_sq = k_star_star - np.dot(k_star, v)


# Output the results
print(f"Predictive mean: {m_star:.4f}")
print(f"Predictive variance: {sigma_star_sq:.4f}")
