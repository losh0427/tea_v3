...
import numpy as np
def draw_trend(file_path, pic_path):
    ...
    print(file_path + '/data_1004.txt')
    objective_value = data["obj_val"].values
    # print(objective_value[431])
    current_max_value = objective_value[0]
    max_index = -1
    # print(objective_value)
    real_data = objective_value
    print(len(objective_value))
    for i in range(1,len(objective_value)):
        # if np.isnan(real_data[i]) :
        #     print(i)    
        if real_data[i] > current_max_value:
            current_max_value = real_data[i]
            max_index = i
            # print(i, current_max_value)
        else:
            real_data[i] = current_max_value
    plt.plot(real_data,'b-',label = 'obj')
    plt.xlabel("number of data")
    plt.ylabel("current_max_obj")
    plt.legend()
    # plt.show()
    plt.savefig( pic_path + "/current_max_obj.png")
    ...
    # plt.legend()
    # plt.savefig( pic_path + "/max_heat_mean_field_value.png")
